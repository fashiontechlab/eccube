...
class Eccube_KernelProdContainer extends Container
{
    ...
    protected static function getDoctrine_Orm_DefaultEntityManagerService($container, $lazyLoad = true)
    {
        ...
        $f = new \Doctrine\ORM\Mapping\Driver\AttributeDriver([(\dirname(__DIR__, 4).'/src/Eccube/Entity'), (\dirname(__DIR__, 4).'/vendor/league/oauth2-server-bundle/src/Entity')], false);
        $g = ($container->privates['annotations.cached_reader'] ?? $container->load('getAnnotations_CachedReaderService'));

        $h = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/src/Eccube/Entity')]);
        $h->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $i = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/Api42/Entity')]);
        $i->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $j = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/Coupon42/Entity')]);
        $j->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $k = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/MailMagazine42/Entity')]);
        $k->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $l = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/ProductReview42/Entity')]);
        $l->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $m = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/Recommend42/Entity')]);
        $m->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $n = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/RelatedProduct42/Entity')]);
        $n->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $o = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/Securitychecker42/Entity')]);
        $o->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));
        $p = new \Eccube\Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Plugin/SiteKit42/Entity')]);
        $p->setTraitProxiesDirectory((\dirname(__DIR__, 4).'/app/proxy/entity'));

        $e->addDriver($f, 'Eccube\\Entity');
        $e->addDriver($f, 'League\\Bundle\\OAuth2ServerBundle\\Entity');
        $e->addDriver(new \League\Bundle\OAuth2ServerBundle\Persistence\Mapping\Driver('League\\Bundle\\OAuth2ServerBundle\\Model\\Client', true, 'oauth2_'), 'League\\Bundle\\OAuth2ServerBundle\\Model');
        $e->addDriver($h, 'Eccube\\Entity');
        $e->addDriver(new \Doctrine\ORM\Mapping\Driver\AnnotationDriver($g, [(\dirname(__DIR__, 4).'/app/Customize/Entity')], false), 'Customize\\Entity');
        $e->addDriver($i, 'Plugin\\Api42\\Entity');
        $e->addDriver($j, 'Plugin\\Coupon42\\Entity');
        $e->addDriver($k, 'Plugin\\MailMagazine42\\Entity');
        $e->addDriver($l, 'Plugin\\ProductReview42\\Entity');
        $e->addDriver($m, 'Plugin\\Recommend42\\Entity');
        $e->addDriver($n, 'Plugin\\RelatedProduct42\\Entity');
        $e->addDriver($o, 'Plugin\\Securitychecker42\\Entity');
        $e->addDriver($p, 'Plugin\\SiteKit42\\Entity');
    ...
    protected static function getSecurity_Firewall_MapService($container)
    {
        ...
        return $container->privates['security.firewall.map'] = new \Symfony\Bundle\SecurityBundle\Security\FirewallMap($a, new RewindableGenerator(function () use ($container) {
            ...
            yield 'security.firewall.map.context.admin' => ($container->privates['.security.request_matcher.2SfIUQz'] ?? $container->load('get_Security_RequestMatcher_2SfIUQzService'));
